// lib/avatar.ts — Gravatar 头像 URL 生成（cravatar.cn 中国镜像）

export function getAvatarUrl(email: string, avatar?: string, size = 80): string {
  // 用户自定义头像优先
  if (avatar) return avatar;

  // Gravatar 默认头像（使用中国镜像 cravatar.cn）
  const hash = md5(email.trim().toLowerCase());
  return `https://cravatar.cn/avatar/${hash}?s=${size}&d=identicon`;
}

/** 简易 MD5 实现（轻量级，Workers 兼容） */
function md5(str: string): string {
  // 使用 Web Crypto API 的替代方案 — 简易纯 JS MD5
  // 对于 Gravatar，我们只需要邮箱的 MD5 哈希
  return md5hex(str);
}
